...
<?php if($generator->generateIndex):?>
    ...
    <?php if (!empty($generator->searchModelClass)): ?>
        ...
        return $this-><?= $generator->renderFunction($generator->ajaxIndex) ?>('index', [
...
<?php if($generator->generateDelete):?>
    ...
    public function actionDelete(<?= $actionParams ?><?=$generator->repositoryParam()?>)
    {
        ...
        <?php if($generator->ajaxDelete):?>
            return $this->ajaxError(Yii::t('flash','<?= strtolower($modelClass) ?>.delete_error'));
        <?php else:?>
            $this->error(Yii::t('flash', '<?= strtolower($modelClass) ?>.delete_error'));
            return $this->goBack();
        <?php endif;?>
        }
    }
